...
def main():
    ...
    # -------------------------------------------------------------
    # 3. Load Broad-19 hd=256 for comparison
    ...
    # -------------------------------------------------------------
    print("\n--- [Step 5] Loading Broad-19 hd=256 for comparison ---")
    ...
    model_configs = [
        ('broad_19_hd256', vqvae_b19, beta_vae_b19, prior_b19),
        ('broad_23_hd256', vqvae_new, beta_vae_new, prior_new),
    ]
    ...
    # -------------------------------------------------------------
    print("\n--- [Step 6] Running Broad-19 vs Broad-23 Evaluation (hd=256) ---")
    ...
    metrics = {
        ...
        'broad_23_hd256': {'vq_acc_p2': 0.0, 'vq_acc_p5': 0.0, 'vae_metric_p2': 0.0, 'vae_metric_p5': 0.0},
    }
    ...
    # Print results table
    print("\n" + "="*70)
    print(f"{'Metric':<20} | {'Broad-19 hd=256':<20} | {'Broad-23 hd=256':<20}")
    print("-" * 70)
    for k in sorted(metrics['broad_19_hd256'].keys()):
        print(f"{k:<20} | {metrics['broad_19_hd256'][k]:20.5f} | {metrics['broad_23_hd256'][k]:20.5f}")
    print("="*70)
    ...
    with open(report_path, 'w') as f:
        f.write("# Broad-19 vs Broad-23 (hd=256) Evaluation\n\n")
        f.write("| Metric | Broad-19 hd=256 | Broad-23 hd=256 |\n")
        f.write("| :--- | :---: | :---: |\n")
        for k in sorted(metrics['broad_19_hd256'].keys()):
            f.write(f"| `{k}` | {metrics['broad_19_hd256'][k]:.5f} | {metrics['broad_23_hd256'][k]:.5f} |\n")
    print(f"Report saved to {report_path}")

    # -------------------------------------------------------------
    # 6. Generate Plot: VQ-VAE Accuracy — Broad-19 vs Broad-23 (hd=256)
    # -------------------------------------------------------------
    ...
    plt.bar(x - width/2, [metrics['broad_19_hd256']['vq_acc_p2']*100, metrics['broad_19_hd256']['vq_acc_p5']*100],
            width, label='Broad-19 hd=256', color='#e91e63')
    plt.bar(x + width/2, [metrics['broad_23_hd256']['vq_acc_p2']*100, metrics['broad_23_hd256']['vq_acc_p5']*100],
            width, label='Broad-23 hd=256', color='#4caf50')

    plt.ylabel('Digit Reconstruction Accuracy (%)', fontweight='bold')
    plt.title('Broad-19 vs Broad-23 at hd=256', fontsize=12, fontweight='bold')
    ...
    # -------------------------------------------------------------
    # 7. Generate Plot: Latent Space PCA — Broad-19 vs Broad-23 (hd=256)
    # -------------------------------------------------------------
    print("Generating Latent Space PCA plots...")
    fig, axes = plt.subplots(2, 2, figsize=(16, 14), dpi=150)

    for row_idx, p_eval in enumerate([2, 5]):
        ...
        for col_idx, (model_name, _, beta_vae_m, _) in enumerate(model_configs):
            ...
            ax.set_title(f"{model_name} ({p_eval}-adic)\nMetric Loss: {metrics[model_name][f'vae_metric_p{p_eval}']:.5f}", fontsize=11, fontweight='bold')
            ax.set_xlabel("PC 1"); ax.set_ylabel("PC 2"); ax.grid(True, alpha=0.3)
            fig.colorbar(sc, ax=ax)

    plt.suptitle("Latent Space PCA — Broad-19 vs Broad-23 (hd=256)", fontsize=14, fontweight='bold')
